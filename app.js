...
const MEDIASOUP_MIN_PORT = process.env.MEDIASOUP_MIN_PORT ? parseInt(process.env.MEDIASOUP_MIN_PORT, 10) : 2000
const MEDIASOUP_MAX_PORT = process.env.MEDIASOUP_MAX_PORT ? parseInt(process.env.MEDIASOUP_MAX_PORT, 10) : 2020
...
const allowedOrigins = [
  ...
  'https://192.168.2.105:5173', // Local network access
  'https://192.168.2.105.nip.io:5173', // nip.io domain for OAuth
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // Allow Elastic Beanstalk domains
...
// Create local uploads directories if they don't exist (dev/backward-compat)
...
// In production (Elastic Beanstalk), terminate TLS at ALB and run the app on HTTP.
// In development, default to HTTP (simpler local testing).
// Set LOCAL_HTTPS=true if you explicitly want local HTTPS with key.pem/cert.pem.
const shouldUseHttps = process.env.NODE_ENV !== 'production' && process.env.LOCAL_HTTPS === 'true'
...
httpServer.listen(PORT, '0.0.0.0', () => {
  console.log(`Server listening on port ${PORT} (${shouldUseHttps ? 'HTTPS' : 'HTTP'}) and bound to 0.0.0.0`)
})
  ...
  try {
    const transportOptions = {
      listenIps: [
        {
          ip: '0.0.0.0',
          // In production, set MEDIASOUP_ANNOUNCED_IP to your instance public IP / elastic IP
          // (or the public IP of your NAT, if applicable).
          announcedIp: MEDIASOUP_ANNOUNCED_IP || '127.0.0.1',
        },
      ],
      enableUdp: true,
      enableTcp: true,
...
peers.on('connection', async (socket) => {
      ...
      try {
        ...
        if (user) {
          ...
          if (roomId && rooms[roomId]) {
            ...
            console.log(`Building profiles map for room ${roomId}. Sockets in room:`, Array.from(socketsInRoom || []))
            if (socketsInRoom) {
              socketsInRoom.forEach(socketId => {
                const socketProfile = socketProfiles.get(socketId)
                console.log(`Socket ${socketId} profile:`, socketProfile)
                if (socketProfile) {
                  profiles[socketId] = socketProfile
                }
              })
            }
            console.log(`Broadcasting profiles to room ${roomId}:`, profiles)
  ...
  socket.on('createRoom', async (data, callback) => {
    ...
    try {
      ...
      audioLevelObserver.on('volumes', ([{ producer }]) => {
        // Only emit if producer is not paused (muted)
  ...
  socket.on('create-poll', async (pollData) => {
    ...
    if (rooms[pollData.roomId] && rooms[pollData.roomId].host === socket.id) {
      try {
        ...
        const rawUserId = userProfile ? userProfile.userId : null;

        // Ensure userId is an integer for the database (PostgreSQL integer column)
        // If it's a UUID or Socket ID (string), we pass null for createdBy
        const dbUserId = (rawUserId && (typeof rawUserId === 'number' || (typeof rawUserId === 'string' && /^\d+$/.test(rawUserId))))
          ? parseInt(rawUserId)
          : null;
        ...
        // Broadcast poll to all participants in the room (including creator)
        peers.to(pollData.roomId).emit('poll-created', broadcastPoll);
        // Also emit to the creator
        socket.emit('poll-created', broadcastPoll);
        console.log(`Poll ${pollData.id} created and broadcasted to room ${pollData.roomId}`);
        console.log(`Room members count: ${rooms[pollData.roomId].peers.size}`);
        console.log(`Broadcasting to sockets in room...`);
        const socketsInRoom = await peers.in(pollData.roomId).allSockets();
        console.log(`Sockets in room ${pollData.roomId}:`, Array.from(socketsInRoom));
      } catch (error) {
  ...
  socket.on('vote-poll', async ({ pollId, optionIndex, roomId }) => {
    ...
    try {
      ...
      peers.to(roomId).emit('poll-vote-update', {
        pollId,
        votes: updatedPoll.votes,
        totalVotes: updatedPoll.totalVotes,
        userVotes: updatedPoll.userVotes
      });
      // Also emit to the voter
      socket.emit('poll-vote-update', {
        pollId,
        votes: updatedPoll.votes,
        totalVotes: updatedPoll.totalVotes,
        userVotes: updatedPoll.userVotes
