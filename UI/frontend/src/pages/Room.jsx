...
const Room = () => {
  ...
  const getCurrentUserInfo = async () => {
    try {
      ...
      if (urlToken) {
        try {
          ...
          const userInfo = {
            ...
            email: decoded.email,
            avatar: resolveAvatarUrl(decoded.avatar_url),
      ...
      if (userEmail && userName) {
        ...
        const apiBase = import.meta.env.VITE_API_URL || `${defaultScheme}://${window.location.hostname}:4000`;
        ...
        if (response.ok) {
          ...
          const userInfo = {
            ...
            avatar: resolveAvatarUrl(data.user.avatar_url),
            id: data.user.id
          };
          setCurrentUserInfo(userInfo);
      ...
      return { name: 'Anonymous', email: '', avatar: resolveAvatarUrl(null) };
    } catch (error) {
  ...
  const getParticipantAvatar = (peerId) => {
    if (!peerId) return resolveAvatarUrl(null);

    // Check if it's the current user
    if (peerId === socket?.id) {
      return resolveAvatarUrl(currentUserInfo?.avatar);
    }
  ...
  function addRemoteMedia(producerId, track, kind, peerId, appData, isNewProducer = false) { // Add isNewProducer parameter
    ...
    if (kind === 'audio') {
      ...
      // Hand raise indicator (shown/hidden via raisedHands effect)
      const handIndicator = document.createElement('div');
      handIndicator.className = 'hand-raise-indicator';
      ...
      // Store VIDEO element with peer ID (active-speaker highlight uses this map)
  ...
  async function toggleScreenShare() {
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getDisplayMedia({
        ...
        audio: {
          // Screen/tab/system audio should NOT use voice-processing.
          // These commonly cause pumping/ducking while screen sharing.
  ...
  useEffect(() => {
    ...
    socket.on('hand-raised', ({ userId, userName, timestamp }) => {
      // Update UI immediately (covers timing where DOM tiles exist but effect hasn't run yet)
      try {
        const tile = remoteVideosRef.current?.querySelector(`.video-container[data-peer-id="${userId}"]`);
        if (tile) {
          let handIndicator = tile.querySelector('.hand-raise-indicator');
          if (!handIndicator) {
        ...
        // no-op; state update below will still drive the UI
      }
    ...
    socket.on('hand-lowered', ({ userId }) => {
      try {
        const tile = remoteVideosRef.current?.querySelector(`.video-container[data-peer-id="${userId}"]`);
        const handIndicator = tile?.querySelector('.hand-raise-indicator');
    ...
    socket.on('hands-cleared', () => {
      try {
        remoteVideosRef.current?.querySelectorAll('.hand-raise-indicator').forEach(el => {
  ...
  // Use a stable room id for UI features (chat/polls). `roomId` is a module variable
  // so components won't re-render when it changes.
  const effectiveRoomId = (displayRoomCode && displayRoomCode !== 'N/A')
    ? displayRoomCode
    : (urlRoomId || null);
  useEffect(() => {
    if (urlRoomId) {
  ...
  useEffect(() => {
    if (remoteVideosRef.current) {
      ...
      videoContainers.forEach(container => {
        ...
        // Ensure the indicator exists even for tiles created before this feature.
        let handIndicator = container.querySelector('.hand-raise-indicator');
        if (!handIndicator) {
          handIndicator = document.createElement('div');
          handIndicator.className = 'hand-raise-indicator';
          handIndicator.textContent = '✋';
          handIndicator.style.display = 'none';
          container.appendChild(handIndicator);
        }
