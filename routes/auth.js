...
import { getPublicBaseUrl, getS3Client } from '../services/s3.js'
...
// Configure multer for avatar upload (in-memory; S3 upload happens in handler)
const upload = multer({
  ...
  try {
    ...
    if (!bucket || !publicBaseUrl) {
      return res.status(500).json({
        ...
        details: 'Set S3_BUCKET (or S3_BUCKET_NAME) and AWS_REGION (and optionally AVATAR_PUBLIC_BASE_URL).'
      })
    }
    ...
    const key = `avatars/user-${userId}/${Date.now()}-${nonce}${safeExt}`
    ...
    const avatarUrl = `${publicBaseUrl}/${key}`
